...
const Mapa = ({ setMapView, setMapSceneView }) => {
  ...
  return (
    <div>
      <button className=" mapbtn" onClick={cambiarVista} >Listado de Capas</button>
      <div id="2d" className={` ${ocultarVisiblisarMapa ? 'visibeMap' : ''}`} ref={mapRef} style={{ height: '80vh', width: '100%' }}>
      </div>
      <div id="3d" ref={mapRef3D} className={` ${ocultarVisiblisarMapa ? '' : 'visibeMap'}`} style={{ height: '80vh', width: '100%' }}>
      </div>
    </div>
